...
* В датасете содиржится информация о 2240 клиентов с 22 признаками. К датасету прилагалось следующее описание:
...
* Выбор и обучение модели для определения вероятности покупки
    ...
    * Сравнение полученных моделей
        * Проверка переообучения модели CATBoost с оверсэмплингом на отложенной выборке
...
* Метрики модели CATBoost с оверсэмплингом близки к 1, а также вид roc-кривой может говорить о переообучении модели.
* Разница между метриками CATBoost с оверсэмплингом для обучающей и валидационной выборки небольшая, в обоих случаях они стремятся к единице. Чтобы исключить вероятность принятия переобученной модели проверили ее на отложенной выборке. Получили, что модель CATBoost с оверсэмплингом была переобучена и на отложенной выборке показала результат хуже чем CATBoost без оверсэмплинга.
* Для предсказания вероятности покупкли клиентами предлагается использовать модель CATBoost без оверсэмплинга.
